...
public class New {
	...
	public static void main(String[] args) throws Exception {
		...
		OWLOntology ontology = manager.loadOntologyFromOntologyDocument(new File((String)JOptionPane.showInputDialog(frame, "Ontology file name: ", "PRO", JOptionPane.PLAIN_MESSAGE)));
		...
		for (OWLSubClassOfAxiom lAx : ontology.axioms(AxiomType.SUBCLASS_OF, Imports.INCLUDED)){
			subAx[axCounter] = lAx;
			axCounter++;
		}
		...
		OWLObjectComplementOf notC = df.getOWLObjectComplementOf(subAx[3].getSubClass());
		...
		OWLObjectUnionOf d2 = df.getOWLObjectUnionOf(df.getOWLObjectComplementOf(subAx[0].getSubClass()), subAx[0].getSuperClass());

		OWLClassExpression rank0 = df.getOWLObjectIntersectionOf(d2, d1, d0);
		OWLClassExpression rank1 = df.getOWLObjectIntersectionOf(d2, d1);

		OWLClassExpression lhs = df.getOWLObjectIntersectionOf(d2, subAx[0].getSubClass());
		OWLClassExpression rhs = subAx[0].getSuperClass();

		OWLSubClassOfAxiom firstQuery = df.getOWLSubClassOfAxiom(rank1, notC);
